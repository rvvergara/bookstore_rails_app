  ...
  describe 'validations' do
    ...
    context 'no current page' do
      it 'is invalid' do
        ...
        expect(collection_item.errors['current_page'])
    ...
    context 'user wants to add same book to collection' do
      it 'raises an error' do
        james = create(:user)
        book = create(:book)
        create(:collection_item, book_id: book.id, user_id: james.id)
        duplicate = build(:collection_item, book_id: book.id, user_id: james.id)
        duplicate.valid?
        expect(duplicate.errors.full_messages.first)
          .to(include('Book can only be added once'))
      end
    end
  end
